...
import com.depromeet.whatnow.api.promiseuser.consts.PROMISE_USER_DELETE_SUCCESS_MESSAGE
import com.depromeet.whatnow.api.promiseuser.consts.PROMISE_USER_REGISTER_SUCCESS_MESSAGE
...
class PromiseUserController(
    ...
    fun joinPromise(@PathVariable("promise-id") promiseId: Long, userId: Long, userLocation: CoordinateVo): ResponseEntity<String> {
        promiseUserRecordUseCase.createPromiseUser(promiseId, userId, userLocation)
        return ResponseEntity(PROMISE_USER_REGISTER_SUCCESS_MESSAGE, HttpStatus.OK)
    }
    ...
    fun cancelPromise(@PathVariable("promise-id") promiseId: Long, @PathVariable("user-id") userId: Long, @PathVariable("status") promiseUserType: PromiseUserType): ResponseEntity<String> {
        promiseUserRecordUseCase.updatePromiseUserType(promiseId, userId, promiseUserType)
        return ResponseEntity(PROMISE_USER_DELETE_SUCCESS_MESSAGE, HttpStatus.OK)
    }
}
